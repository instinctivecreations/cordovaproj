...
(function() {
  'use strict';
  var Promise, common, debug, extractOrder, flatten, fs, generate, join, minify, promisify, sort, sortAlphabetically, sortByAfter, sortByBefore, sortByConfig, sortByVendor, sysPath,
  ...
  promisify = require('fantasy-promises/node');
  ...
  join = function(path, type, files, wrapper) {
    var joined;
  ...
  minify = function(data, optimizer, isEnabled, callback) {
    if (isEnabled) {
      return optimizer(data, path, callback);
    } else {
      return callback(null, data);
    }
  };

  generate = function(path, sourceFiles, config, callback) {
    ...
    return minify(joined, optimizer, config.optimize, function(error, data) {
      if (error != null) {
