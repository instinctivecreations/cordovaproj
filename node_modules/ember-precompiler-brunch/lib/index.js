...
(function() {
  ...
  module.exports = EmberPrecompiler = (function() {
    ...
    EmberPrecompiler.prototype.compile = function(data, path, callback) {
      ...
      try {
        content = this.precompileEmber(data);
        path = path.replace(new RegExp('\\\\', 'g'), '/').replace(/^app\//, '').replace(/^templates\//, '').replace(/\.\w+$/, '');
        return result = "Ember.TEMPLATES[" + (JSON.stringify(path)) + "] = Ember.Handlebars.template(" + content + ");\nmodule.exports = module.id;";
      } catch (e) {
        return error = e;
      } finally {
    ...
    EmberPrecompiler.prototype.precompileEmber = function(data) {
      return this.window.Ember.Handlebars.precompile(data);
    };
